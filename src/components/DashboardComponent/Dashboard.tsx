...
useEffect(() => {
  async function fetchReports() {
    try {
      console.log("Starting report fetch from dashboard");
...
const handleStatusUpdate = async (
 ...
 function determineReportType(report: any): Report['type'] {
   ...
   if (report.accountUsername) return 'vendor';
   return 'social-media';
 }
...
<Tabs defaultValue="all" className="w-full">
      ...
      <div className="relative w-full mb-4">
        <div className="overflow-x-auto scrollbar-hide -mx-2 px-2">
          <TabsList className="inline-flex min-w-full w-max border rounded-lg bg-muted p-1">
            <TabsTrigger 
              value="all" 
       ...
       {['all', 'romance', 'platform', 'vendor'].map((tab) => (
